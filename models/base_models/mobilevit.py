...
__all__ = ['get_mobilevit_x_small', 'get_mobilevit_small', 'get_mobilevit_xx_small']
...
class MobileViT(nn.Module):
    # ["": out_channels, stride, transformer_dim, ffn_dim, num_transformer_blocks, expand_ratio=4, dilation=1]
    
    arch_settings = {
        'small': [
            ['mobilenetv2', 32, 1, 1, 4], ['mobilenetv2', 64, 2, 3, 4], ['mobilevit', 96, 2, 144, 288, 2, 4],
            ['mobilevit', 128, 1, 192, 384, 4, 4, 2], ['mobilevit', 160, 1, 240, 480, 3, 4, 4],
        ],
        'x_small': [
            ['mobilenetv2', 32, 1, 1, 4], ['mobilenetv2', 48, 2, 3, 4], ['mobilevit', 64, 2, 96, 192, 2, 4],
            ['mobilevit', 80, 1, 120, 240, 4, 4, 2], ['mobilevit', 96, 1, 144, 288, 3, 4, 4],
        ],
        'xx_small': [
            ['mobilenetv2', 16, 1, 1, 2], ['mobilenetv2', 24, 2, 3, 2], ['mobilevit', 48, 2, 64, 128, 2, 2],
            ['mobilevit', 64, 1, 80, 160, 4, 2, 2], ['mobilevit', 80, 1, 96, 192, 3, 2, 4],
        ],
    }
    def __init__(self, arch='small', in_channels=3, stem_channels=16, last_exp_factor=4, out_indices=(0, 1, 2, 3, 4), norm_cfg=dict(type='SyncBatchNorm'), 
    ...
    '''makemobilevitlayer'''
    @staticmethod
    def makemobilevitlayer(in_channels, norm_cfg, act_cfg, out_channels, stride, transformer_dim, ffn_dim, num_transformer_blocks, expand_ratio=4, dilation=1):
        layer = []
        # in_channels, out_channels, stride, expand_ratio, dilation=1, norm_cfg=None, act_cfg=None
        layer.append(InvertedResidual(
            in_channels=in_channels, out_channels=out_channels, stride=stride, expand_ratio=expand_ratio, dilation=dilation, act_cfg=act_cfg, norm_cfg=norm_cfg,
        ))
...
def get_mobilevit_x_small(pretrained=False):
    model = MobileViT(arch='x_small', pretrained_model_path=pretrained)
    return model

def get_mobilevit_small(pretrained=False):
    model = MobileViT(arch='small', pretrained_model_path=pretrained)
    return model

def get_mobilevit_xx_small(pretrained=False):
    model = MobileViT(arch='xx_small', pretrained_model_path=pretrained)
    return model
