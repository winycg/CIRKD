...
__all__ = ['get_deeplabv3_mobilenet_ssseg']


class DeepLabV3(nn.Module):

    def __init__(self, nclass, backbone='mobilenetv3_small', pretrained_base=True, **kwargs):
        super(DeepLabV3, self).__init__()
        
        if backbone == 'mobilenetv3_small':
            self.pretrained = get_mobilenet_v3_small(pretrained=pretrained_base)
            self.head = _DeepLabHead(576, nclass, **kwargs)
        elif backbone == 'mobilenetv3_large':
            self.pretrained = get_mobilenet_v3_large(pretrained=pretrained_base)
            self.head = _DeepLabHead(960, nclass, **kwargs)
        elif backbone == 'mobilenetv2_ssseg':
            self.pretrained = get_mobilenet_v2_ssseg(pretrained=pretrained_base)
            self.head = _DeepLabHead(320, nclass, **kwargs)
        else:
            raise KeyError('no such backbone')
...
def get_deeplabv3_mobilenet_ssseg(backbone='mobilenetv3_small', local_rank=None, pretrained=None, 
                  pretrained_base=True, num_class=19, **kwargs):
    model = DeepLabV3(num_class, backbone=backbone, pretrained_base=pretrained_base, **kwargs)
